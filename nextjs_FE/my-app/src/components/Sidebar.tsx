...
// Logo Component
const Logo = () => (
  <Link href="/" aria-label="Acet Labs" className="font-normal flex space-x-2 items-center text-sm text-black dark:text-white py-1">
...
const LogoIcon = () => (
  <Link href="/" aria-label="Acet Labs Logo" className="font-normal flex space-x-2 items-center text-sm text-black dark:text-white py-1">
    <div className="h-5 w-6 bg-black dark:bg-white rounded-br-lg rounded-tr-sm rounded-tl-lg rounded-bl-sm flex-shrink-0" />
  </Link>
);
      ...
      <SidebarBody className="justify-between gap-10">
        <div className="flex flex-col flex-1 overflow-y-auto">
          ...
          <div className="mt-8 flex flex-col gap-2">
            {links.map((link, idx) => (
              <Link key={idx} href={link.href} className="flex items-center gap-2 p-2 hover:bg-gray-200 rounded-lg">
                {link.icon}
